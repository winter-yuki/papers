...
%    \end{itemize}
    ...
    This approach requires effect handlers feature support and distances effect system from direct control over resource use (which may be the point that resources should never be used directly, only via high-level API).
    Semantics of such an effect system can be formulated with context readings of types.

    \begin{minted}{scala}
        def buildString(ident : Int)
          { f : Unit => Unit / {Emit[String]} }
          : String / {Format}
        //          provided ?\big\uparrow? capabilities
        //         ?\big\uparrow? required capabilities
    \end{minted}
