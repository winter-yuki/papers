...
%    \end{itemize}
    ...
    Boxing provides some degree of flexibility, however the result is still far from desired.
    Specifically, without effect variables support boxed blocks cannot be composed.
    Moreover, boxing on term level should be explicit and, to our knowledge, there is no developed box inference.
%    \begin{minted}[escapeinside=##]{scala}
%        def compose (f: B => C as {?}, g: A => B at {?}):
%    \end{minted}

    TODO disadvantages % todo
    ...
    TODO \cite{odersky2022scoped, boruch2023capturing} % todo
