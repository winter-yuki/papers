...
%    \end{itemize}
    ...
    Firstly, the effect system should prevent accidental handling, i.e.,\ it should explicitly track information that particular effects will be handled, e.g.,\ in the previous example, \texttt{f} should carry the information about expected \mintinline{scala}|FileException|.
    ...
    For example, a function in Koka that transforms a generator requires many ceremonies to type-check:
    \begin{minted}{haskell}
        fun reyield(
            f : (int) -> <|e> int,
            g : () -> <yield,yield|e> ()
    ...
    Note that syntactic pollution with the capture-checking approach seems quite reasonable.
    Users need to pay the syntactic burden only when leaking something out of scope; otherwise, capability inference should do the job.
    At the same time, average programmers are generally not familiar with dependent types, which may complicate the adoption of this feature.
