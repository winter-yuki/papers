...
%    \end{itemize}
    ...
    So second-class value can be boxed, leaked from the current context, carried to another context, and, if other context is appropriate, unboxed to use.
    \begin{gather*}
    ...
    To provide users an ability to refer to capabilities implicitly captured by some value, Effekt features capability polymorphism as a way to express: a boxed value closes over the same capabilities as other particular value captures.
    \begin{minted}{scala}
        def map[T, R] { f: T => R }
          : List<T> => R at {f}
          { box { xs => go(f, xs) } }
